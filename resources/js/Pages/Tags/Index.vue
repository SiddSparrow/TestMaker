                ...
                <div v-else class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                    <div v-for="tag in filteredTags" :key="tag.id"
                        ...
                        <div class="flex flex-col gap-1 opacity-0 group-hover:opacity-100 focus-within:opacity-100 transition-opacity">
                            ...
                            <BaseButton variant="outline" size="sm" icon-only :aria-label="`Excluir etiqueta: ${tag.name}`"
                                        class="!text-red-600 !border-red-300 hover:!bg-red-50" @click="deleteTag(tag)">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
